<?php
    ...
    class Sales extends Validator{
        ...
        public function readDetailSale(){
            ...
            INNER JOIN categories AS C ON C.id = P.id_category WHERE id_customer = :id';
            $params = array($this->idCustomer);
            return Database::getRows($sql, $params);
        }

        public function readSale(){
            $sql = 'SELECT S.id AS id, C.name AS client, P.name AS product, S.status AS status, S.date AS date FROM sales AS S
            INNER JOIN sales_detail as SD on S.id = SD.id_sale 
            INNER JOIN products AS P ON P.id = SD.id_product
            INNER JOIN customers as C on C.id = S.id_customer';
            $params = array();
            return Database::getRows($sql, $params);
        }
    }
